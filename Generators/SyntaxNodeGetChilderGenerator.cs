...
#pragma warning disable IDE0063 // Use simple 'using' statement: we want to control when the 'using' varibales go out of scope
            ...
            var separatedSyntaxListType = compilation.GetTypeByMetadataName("CompilerCSharpLibrary.CodeAnalysis.Syntax.SeparatedSyntaxList`1");
            var syntaxNodeType = compilation.GetTypeByMetadataName("CompilerCSharpLibrary.CodeAnalysis.Syntax.SyntaxNode");

            if (listType == null || separatedSyntaxListType == null || syntaxNodeType == null)
                return;
            ...
            using (var indentedTextWriter = new IndentedTextWriter(stringWriter, indentString))
            {
                ...
                using (var nameSpaceCurly = new CurlyIndenter(indentedTextWriter, "namespace CompilerCSharpLibrary.CodeAnalysis.Syntax"))
                {
                                    ...
                                    if (IsDerivedFrom(propertyType, syntaxNodeType))
                                    {
                                        ...
                                        if (canBeNull)
                                            indentedTextWriter.Indent--;
                                             ...
                                             SymbolEqualityComparer.Default.Equals(propertyType.OriginalDefinition, listType))
                                    {
